...
import br.com.builders.api.integration.cliente.adapter.jpa.ClienteQueryRepository;
import br.com.builders.api.integration.cliente.app.SpringAppApplication;
import br.com.builders.api.integration.cliente.handle.Error;
import br.com.builders.api.integration.cliente.representation.model.ClienteDTO;
...
class TestsClienteCadastro {
    ...
    void naoDeveCadastraClienteComNomeJaCadastrado() {
        ...
        final var cliente= clienteQueryRepository.findAll().stream ().findFirst ();
        response = restTemplate.postForEntity(getPath(), cliente, Error.class);
        assertNotNull(response);
        assertTrue(
